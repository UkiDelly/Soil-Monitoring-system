  ...
  createHistory() {
    ...
    if (sensorDataList.length < 7) {
      for (double i = 0; i < sensorDataList.length; i++) {
        ...
        nSpot.add(FlSpot((i+1), sensorDataList[i.toInt()]['nitrogen'].toDouble()));
        pSpot.add(
            FlSpot(
            (i + 1), sensorDataList[i.toInt()]['phosphorous'].toDouble()));
        kSpot.add(FlSpot((i + 1), sensorDataList[i.toInt()]['potassium'].toDouble()));
        phSpot.add(FlSpot((i + 1), sensorDataList[i.toInt()]['pH'].toDouble()));
        tempSpot.add(
            FlSpot((i+1), sensorDataList[i.toInt()]['temperature'].toDouble()));
        moistureSpot
            .add(FlSpot((i + 1), sensorDataList[i.toInt()]['moisture'].toDouble()));
        humiditySpot
            .add(FlSpot((i + 1), sensorDataList[i.toInt()]['humidity'].toDouble()));
      }
    } else {
      ...
      for (double i = 0; i < sensorDataList.length; i++) {
        ...
        pSpot.add(FlSpot((i + 1), reverseList[i.toInt()]['phosphorous'].toDouble()));
        kSpot.add(FlSpot((i + 1), reverseList[i.toInt()]['potassium'].toDouble()));
        phSpot.add(FlSpot((i + 1), reverseList[i.toInt()]['pH'].toDouble()));
        tempSpot
            .add(FlSpot((i + 1), reverseList[i.toInt()]['temperature'].toDouble()));
        moistureSpot
